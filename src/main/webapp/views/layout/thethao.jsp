		...
		<div class="news-item"
			...
			<div class="news-content" style="flex: 1;">
				...
				<p class="news-desc"
					...
					<c:choose>
						<c:when test="${fns:length(tt.content) > 100}">
                        ${fns:substring(tt.content,0,100)}...
                    </c:when>
						<c:otherwise>${tt.content}</c:otherwise>
					</c:choose>
				</p>
				<p class="news-meta"
					style="font-style: italic; color: gray; font-size: 14px;">
					Ngày đăng:
					<fmt:formatDate value="${tt.postedDate}" pattern="dd/MM/yyyy" />
					| Tác giả: ${tt.author} | Lượt xem: ${tt.viewCount}
				</p>
			</div>
		</div>
	</c:forEach>
