...
<body>
	...
	<c:forEach var="dm" items="${newsList}">
		<div class="news-item"
			...
			<div class="news-img"
				...
				<c:choose>
					<c:when test="${not empty dm.image}">
						<a href="${path}/chitiet?id=${dm.id}"> <img
							src="${path}/image/${dm.image}" width="120" height="90" alt="Ảnh" />
						</a>
					</c:when>
					<c:otherwise>
						<a href="${path}/chitiet?id=${dm.id}"
			...
			<div class="news-content" style="flex: 1;">
				<a href="${path}/chitiet?id=${dm.id}" class="news-title"
					style="color: #2563EB; font-weight: bold; text-decoration: none;">
					${dm.title} </a>
				<p class="news-desc"
					style="font-style: italic; color: #333; margin: 4px 0;">
					<c:choose>
						<c:when test="${fns:length(dm.content) > 100}">
                        ${fns:substring(dm.content,0,100)}...
                    </c:when>
						<c:otherwise>${dm.content}</c:otherwise>
					</c:choose>
				</p>
				<p class="news-meta"
					style="font-style: italic; color: gray; font-size: 14px;">
					Ngày đăng:
					<fmt:formatDate value="${dm.postedDate}" pattern="dd/MM/yyyy" />
					| Tác giả: ${dm.author} | Lượt xem: ${dm.viewCount}
				</p>
			</div>
		</div>
	</c:forEach>
