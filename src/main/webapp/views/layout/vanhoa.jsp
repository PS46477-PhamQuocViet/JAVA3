...
<body>
	...
	<c:forEach var="vh" items="${newsList}">
		<div class="news-item"
			...
			<div class="news-img"
				...
				<c:choose>
					<c:when test="${not empty vh.image}">
						<a href="${path}/chitiet?id=${vh.id}"> <img
							src="${path}/image/${vh.image}" width="120" height="90" alt="Ảnh" />
						</a>
					</c:when>
					<c:otherwise>
						<a href="${path}/chitiet?id=${vh.id}"
			...
			<div class="news-content" style="flex: 1;">
				...
				<p class="news-desc"
					...
					<c:choose>
						<c:when test="${fns:length(vh.content) > 100}">
                        ${fns:substring(vh.content,0,100)}...
                    </c:when>
						<c:otherwise>${vh.content}</c:otherwise>
					</c:choose>
				</p>
				<p class="news-meta"
					style="font-style: italic; color: gray; font-size: 14px;">
					Ngày đăng:
					<fmt:formatDate value="${vh.postedDate}" pattern="dd/MM/yyyy" />
					| Tác giả: ${vh.author} | Lượt xem: ${vh.viewCount}
				</p>
			</div>
		</div>
	</c:forEach>
